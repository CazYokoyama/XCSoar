    ...
    [[maybe_unused]] const InfoBoxLook &look_infobox) noexcept
{
  ...
  // e_WP_H
  static StaticString<20> waypoint_altitude_arrive_s;
  double waypoint_altitude_arrive;
  if (tp == TaskType::ORDERED)
    waypoint_altitude_arrive = calculated.ordered_task_stats.current_leg
                                   .solution_remaining.GetArrivalAltitude();
  else
    waypoint_altitude_arrive = calculated.task_stats.current_leg
                                   .solution_remaining.GetArrivalAltitude();
  FormatAltitude(waypoint_altitude_arrive_s.data(), waypoint_altitude_arrive,
                 Units::GetUserAltitudeUnit(), true);
  text_pixel = (int)font.TextSize(waypoint_altitude_arrive_s).width;
  if (text_pixel <= for_waypoint_info) {
